...
fn process_rib_file(query: &QueryIr, db: &AsRelDb, rib_file: &Path) -> Result<()> {
    ...
    let route_stats_filename = format!("all5/{collector}--route_stats5.csv.gz");
    let route_first_hop_stats_filename = format!("all5/{collector}--route_first_hop_stats5.csv.gz");
    let as_stats_filename = format!("all5/{collector}--as_stats5.csv.gz");
    let as_pair_stats_filename = format!("all5/{collector}--as_pair_stats5.csv.gz");
    if [
