...
fn process_rib_file(query: &QueryIr, db: &AsRelDb, rib_file: &Path) -> Result<()> {
    ...
    let route_stats_filename = format!("all4/{collector}--route_stats3.csv.gz");
    let route_first_hop_stats_filename = format!("all4/{collector}--route_first_hop_stats3.csv.gz");
    let as_stats_filename = format!("all4/{collector}--as_stats3.csv.gz");
    let as_pair_stats_filename = format!("all4/{collector}--as_pair_stats3.csv.gz");
    if [
