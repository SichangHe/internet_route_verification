"""Run at `scripts/` with `python3 -m scripts.stats.as_pair_spec_all_all_some`.

Adopted from `as_spec_all_all_some`.
"""
...
def main() -> None:
    ...
    n_meh = len(df)
    percentage = n_meh * 100 / n_as_pair
    print(f"{n_meh} AS pairs with specal cases, {percentage:.1f}%.")
